...
const api = axios.create({
  ...
  headers: {
    ...
    // Lets the backend record which client a user is active on (web vs app)
...
// Helper to manage the refresh token in sessionStorage (cleared on browser close)
export const setRefreshToken = (token) => {
  if (typeof window !== 'undefined') {
    if (token) {
      // Keep existing sessionStorage behavior for in-page reads
      sessionStorage.setItem('refresh_token', token);
      // Also set a session cookie so Edge middleware can detect auth on navigation
      // Use a session cookie (no expires) and make it available site-wide
      try {
...
export const getRefreshToken = () => {
  if (typeof window !== 'undefined') {
    // Prefer sessionStorage (fast), fall back to cookie for cases like cross-tab
