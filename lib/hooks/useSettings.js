...
export function useSettings() {
  ...
  const handleProfilePicUpload = async (file) => {
    ...
    try {
      ...
      const key = `profile-pics/${user.id}-${Date.now()}.${fileExt}`;
      const proxyUrl = await uploadToR2(file, key);
      const directUrl = getDisplayUrl(proxyUrl);

      const oldPicUrl = user.profile_pic;
      await apiRequest('/users/profile', {
        method: 'PATCH',
        body: { profile_pic: directUrl }
      });
      await checkUser();

      if (oldPicUrl) {
        try { await deleteFromR2(oldPicUrl); }
